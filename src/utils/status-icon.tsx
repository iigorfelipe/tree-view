...
const EnergyIcon = ({ status }: EnergyIconProps) => {
  return (
    <svg width="9" height="12" viewBox="0 0 9 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        fill={status === 'alert' ? '#ED3833' : '#52C41A'}
      />
    </svg>
  );
};
...
export const StatusIcon = ({ componentSelected }: StatusIconProps) => {
  ...
  if (componentSelected.sensorType === 'energy') {
    return <EnergyIcon status={componentSelected.status || 'operating'} />;
  }

  if (componentSelected.sensorType === 'vibration') {
    return (
      <img
        src={componentSelected.status === 'alert' ? '/tree-view/alert.svg' : '/tree-view/operating.svg'}
